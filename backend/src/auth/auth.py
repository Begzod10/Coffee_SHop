...
AUTH0_DOMAIN = 'rimefara.us.auth0.com'
ALGORITHMS = ['RS256']
...
def get_token_auth_header():
    header_of_auth = request.headers.get('Authorization', None)
    if not header_of_auth:
        raise AuthError({"code": "Authorization_header_missing",
                         "description": "Authorization header is expected"}, 401)
    header_parts = header_of_auth.split(' ')
...
def check_permissions(permission, payload):
    ...
    if permission not in payload.get('permission'):
