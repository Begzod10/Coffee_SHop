...
def change_drink_info(payload, id):
    ...
    try:
        request_title = request_input.get('title')
        request_recipe = request_input.get('recipe')
        if request_title:
            drink.title = request_title

        if request_recipe:
            drink.recipe = json.dumps(request_input['recipe'])
